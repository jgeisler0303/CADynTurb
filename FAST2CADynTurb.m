...
tw_sid.De.M0= 2*sqrt(tw_sid.Me.M0*tw_sid.Ke.M0) * diag([GetFASTPar(edtwrDataOut, 'TwrFADmp(1)') GetFASTPar(edtwrDataOut, 'TwrSSDmp(1)')])/100;
...
phi_abs= edbldDataOut.BldProp(:, find(strcmp(edbldDataOut.BldPropHdr, 'PitchAxis')))/180*pi;
phi= [0; diff(phi_abs(:))]; % add phi_abs(1) to pitch angle
...
bd_sid.De.M0= 2*sqrt(bd_sid.Me.M0*bd_sid.Ke.M0) * diag([GetFASTPar(edbldDataOut, 'BldFlDmp(1)') GetFASTPar(edbldDataOut, 'BldEdDmp(1)')])/100;

%  write_sid_maxima(bd_sid, [base_path '_bd_sid'], 'tower', [], 1e-5, 1)
tname= [tempname '.m'];
write_sid_maxima(bd_sid, tname, 'blade', [], 1e-5, 2);
param= load2struct(tname, param);
delete(tname)
