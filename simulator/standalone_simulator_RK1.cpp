...
int main(int argc, char* argv[]) {
    RK1condensed<MODEL_NAME> system;
    FAST_Wind* wind;
    double simtime;
    double simstep;
    double rpm0;
    ...
    ("c,config", "Options file for the integration algorithm (default: rk1_options.txt)", cxxopts::value<std::string>()->default_value("rk1_options.txt"))
    ("v,verbose", "Print verbose output from the integration algorithm", cxxopts::value<bool>()->default_value("false"))
    ...
    const bool verbose = argc_result["verbose"].as<bool>();
...
template<class rk1_integrator>
void updateDISCON(rk1_integrator &system, DISCON_Interface &DISCON) {
    ...
    if constexpr (has_phi_gen_d<typename MODEL_NAME::states_t>::value) {
    ...
    if constexpr (has_theta1<typename MODEL_NAME::inputs_t>::value) {
    ...
    if constexpr (has_tow_fa_dd<typename MODEL_NAME::states_t>::value) {
        DISCON.f_a_acc= system.states.tow_fa_dd;
    } else {
        DISCON.f_a_acc= 0.0;
    }
    if constexpr (has_tow_ss_dd<typename MODEL_NAME::states_t>::value) {
...
template<class rk1_integrator> // necessary to make "if constexpr (has_" work correctily
bool simulate(rk1_integrator &system,
              FAST_Wind* wind,
              double ts,
              double tfinal,
              const std::string &discon_path,
    ...
    out.setTime(0.0, ts);
    setupOutputs(out, static_cast<MODEL_NAME&>(system));
    ExtraSensors<MODEL_NAME> extraSensors(out, static_cast<MODEL_NAME&>(system));
    
    system.t= 0.0;

    system.inputs.vwind= wind_adjust*wind->getWind(system.t);
    if constexpr (has_h_shear<typename MODEL_NAME::inputs_t>::value) {
    ...
    if constexpr (has_phi_gen<typename MODEL_NAME::states_t>::value) {
        system.states.phi_gen= -system.inputs.Tgen*system.param.GBRatio*system.param.GBRatio/system.param.DTTorSpr;
    }
    
    system.locked_states[system.states_idx.phi_rot]= true;
    if constexpr (has_phi_gen<typename MODEL_NAME::states_t>::value) {
        system.locked_states[system.states_idx.phi_gen]= true;
    }
    ...
    system.locked_states[system.states_idx.phi_rot]= false;
    if constexpr (has_phi_gen<typename MODEL_NAME::states_t>::value) {
        system.locked_states[system.states_idx.phi_gen]= false;
    }
    ...
    while(system.t<tfinal)    {
        ...
        if (verbose) {
            std::cout
                ...
                << " backsteps=" << system.n_back_steps
                << " substeps=" << system.sub_iter
                << std::endl;
        }
...
template<class rk1_integrator>
bool DISCON_Step(double t, DISCON_Interface &DISCON, rk1_integrator &system) {
