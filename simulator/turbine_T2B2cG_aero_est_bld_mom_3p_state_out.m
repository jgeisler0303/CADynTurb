function varargout= turbine_T2B2cG_aero_est_bld_mom_state_out(varargin)
...
if length(varargin)==2
    ...
    x(tow_fa_d_idx, :)= d_in.QD_TFA1.Data;      % 8
    x(tow_ss_d_idx, :)= -d_in.QD_TSS1.Data;     % 9
    x(bld_flp_d_idx, :)= d_in.QD_BF1.Data;      % 10
    x(bld_edg_d_idx, :)= d_in.QD_BE1.Data;      % 11
    x(phi_rot_idx, :)= d_in.LSSTipVxa.Data/30*pi; %12
    x(Dphi_gen_d_idx, :)= -(d_in.QD_DrTr.Data + d_in.QD_TSS1.Data*param.TwTrans2Roll)*param.GBRatio; %13
    x(vwind_d_idx, :)= 0;
    ...
    y(4, :)= d_in.RootMxb.Data*1000;
    y(5, :)= d_in.RootMyb.Data*1000;
    ...
    d_out= addts(d_out, 'RootMxb', 'kNm', y(out_bld_flp_mom_idx, :)/1000);
    d_out= addts(d_out, 'RootMyb', 'kNm', y(out_bld_edg_mom_idx, :)/1000);
    d_out= addts(d_out, 'y', '-', y);
    d_out= addts(d_out, 'q', '-', x);
    d_out= addts(d_out, 'dq', '-', dx);
    d_out= addts(d_out, 'ddq', '-', ddx);
