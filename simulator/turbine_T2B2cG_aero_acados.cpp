...
int main(int argc, char* argv[]) {
    sim_solver_capsule *capsule = T2B2cG_aero_acados_sim_solver_create_capsule();
    FAST_Wind* wind;
    double simtime;
    double simstep;
    ...
    if(int status= T2B2cG_aero_acados_sim_create(capsule)) {
        printf("acados_create() returned status %d. Exiting.\n", status);
        exit (EXIT_FAILURE);                    
    }
    
//     int tmp_int = 2;
//     sim_opts_set(T2B2cG_aero_sim_config, T2B2cG_aero_sim_opts, "newton_iter", &tmp_int);
//     tmp_int = 2;
//     sim_opts_set(T2B2cG_aero_sim_config, T2B2cG_aero_sim_opts, "num_stages", &tmp_int);
//     tmp_int = 1;
//     sim_opts_set(T2B2cG_aero_sim_config, T2B2cG_aero_sim_opts, "num_steps", &tmp_int);
    {
        ...
        try {
            ...
            if(int status= T2B2cG_aero_acados_sim_free(capsule)) {
                printf("T2B2cG_aero_acados_sim_free() returned status %d. \n", status);
            }
            T2B2cG_aero_acados_sim_solver_free_capsule(capsule);
            exit (EXIT_FAILURE);
        }
...
//             p_out << p[i] << std::endl;
//         p_out.close();
        T2B2cG_aero_acados_sim_update_params(capsule, p, 29655);
...
bool simulate(sim_solver_capsule *capsule, FAST_Wind* wind, double ts, double tfinal, const std::string &discon_path, const std::string &out_file_name) {
    ...
    sim_config *acados_sim_config = T2B2cG_aero_acados_get_sim_config(capsule);
    sim_in *acados_sim_in = T2B2cG_aero_acados_get_sim_in(capsule);
    sim_out *acados_sim_out = T2B2cG_aero_acados_get_sim_out(capsule);
    void *acados_sim_dims = T2B2cG_aero_acados_get_sim_dims(capsule);
    ...
    for(int i_step= 0; i_step<n_steps; ++i_step) {
        ...
        int status = T2B2cG_aero_acados_sim_solve(capsule);
