function [d_out, cpu_time, int_err, n_steps, n_backsteps, n_sub_steps]= sim_turbine_T2B2cG_aero_est_bld_mom(d_in, param, step_predict, do_est, Q, R, N, T_adapt)
...
x(Dphi_gen_idx)= u(in_Tgen_idx)*param.GBRatio/param.DTTorSpr;
dx(Dphi_gen_idx)= 0;
...
[x0, dx0, ddx0, ~, ~, ~, res]= turbine_T2B2cG_aero_est_bld_mom_mex(x, dx, u, param, 0, opts_init);
if ~res
...
for i= 2:nt
    ...
    [x(:, i), dx(:, i), ddx(:, i), y_pred(:, i), AB, CD, res, cpu_time(i), int_err(i), n_steps(i), n_backsteps(i), n_sub_steps(i)]= ...
        turbine_T2B2cG_aero_est_bld_mom_mex(x_in, dx_in, u(:, i-1), param, t(i)-t(i-1), opts);
...
if step_predict
    ...
    d_out= turbine_T2B2cG_aero_est_bld_mom_state_out(t, x, dx, ddx, u, y_pred, param);
end