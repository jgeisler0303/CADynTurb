function [d_out, cpu_time, int_err, n_steps, n_backsteps, n_sub_steps, Q, R]= sim_turbine_T1B1i_aero_est(d_in, param, step_predict, do_est, Q, R, N, T_adapt, adjust_adapt)
...
if do_est
    q0= [0 0 0 0 0 8 0 0];
    dq0= [0 0 0 0 1000/30*pi/param.GBRatio 0 0 0];
    ddq0= [0 0 0 0 0 0 0 0];
else
    q0= [d_in.Q_TFA1.Data(1) d_in.Q_B1F1.Data(1) d_in.Q_B2F1.Data(1) d_in.Q_B3F1.Data(1) 0 d_in.RtVAvgxh.Data(1) d_in.RtHSAvg.Data(1) d_in.RtVSAvg.Data(1)];
    dq0= [0 0 0 0 1000/30*pi/param.GBRatio 0 0 0];
    ddq0= [0 0 0 0 0 0 0 0];
end
...
[x_ref, y_meas]= turbine_T1B1i_aero_est_state_out(d_in, param);
...
for i= 2:nt
    ...
    [q(:, i), dq(:, i), ddq(:, i), y_pred(:, i), AB, CD, res, cpu_time(i), int_err(i), n_steps(i), n_backsteps(i), n_sub_steps(i)]= ...
        turbine_T1B1i_aero_est_mex(x_in, dx_in, u(:, i-1), param, t(i)-t(i-1), opts);
    ...
    if do_est
        [q(:, i), dq(:, i), Sigma_est, Q, R]= est_T1B1i_aero_est(q(:, i), dq(:, i), u(:, i-1), y_pred(:, i), y_meas(:, i), param, Sigma_est, AB, CD, Q, R, N, t(i), alpha_adapt, adjust_adapt);
%         for j= 3:8
%             R(j, j)= R(j, j)*1.2;
%         end
    end
...
if step_predict
    ...
    d_out= turbine_T1B1i_aero_est_state_out(t, q, dq, ddq, u, y_pred, param);
end