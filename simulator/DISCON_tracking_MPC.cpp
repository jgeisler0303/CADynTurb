...
typedef EXPAND_AND_CONCAT(OPC_PREFIX,Parameters) Parameters_t;
#include "calc_tracking_references.hpp"
...
void init_MPC(avrSwap_t *avrSwap, int32_t *aviFail, char *accInfile, char *avcOutname, char *avcMsg) {
    ...
    int max_msg_char = avrSwap->max_msg_char;
    ...
    if(ocp_params.unsetParamsWithMsg()) {
        char msg[max_msg_char];
        snprintf(msg, max_msg_char, "Not all OCP parameters could be read from the file. Terminating.");
        for(auto const &i : ocp_params.info_map) {
            if(!i.second.isSet) {
                const size_t len = strnlen(msg, max_msg_char);
                if(len < static_cast<size_t>(max_msg_char) - 1) {
                    snprintf(msg + len, max_msg_char - len, "\n%s", i.first.c_str());
                }
            }
        }
        strncpy(avcMsg, msg, max_msg_char);
        aviFail[0]= 1;
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free)(acados_ocp_capsule);
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free_capsule)(acados_ocp_capsule);
        acados_ocp_capsule= nullptr;
    ...
    if(static_cast<int>(vec.size()) != NP) {
        snprintf(avcMsg, max_msg_char,
            "Parameter layout mismatch: loaded %zu parameters, acados solver expects NP=%d. "
            "Likely wrong *_param.hpp for this c_generated_code.",
            vec.size(), NP);
        aviFail[0]= 1;
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free)(acados_ocp_capsule);
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free_capsule)(acados_ocp_capsule);
    ...
    for (int i = 0; i <= N; i++) {
        ...
        if (status) {
            snprintf(avcMsg, max_msg_char, EXPAND_AND_STRINGIFY(OPC_PREFIX)"_acados_acados_update_params() returned status %d. Terminating.", status);
            aviFail[0]= 1;
            EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free)(acados_ocp_capsule);
            EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free_capsule)(acados_ocp_capsule);
            acados_ocp_capsule= nullptr;
    ...
    const char *ocp_setup_err = further_ocp_setup(acados_ocp_capsule, accInfile);
    if(ocp_setup_err) {
        snprintf(avcMsg, max_msg_char, "Further OCP setup failed: %s Terminating.", ocp_setup_err);
        aviFail[0]= 1;
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free)(acados_ocp_capsule);
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free_capsule)(acados_ocp_capsule);
        acados_ocp_capsule= nullptr;
    ...
    if(ekf.system.param.unsetParamsWithMsg()) {
        snprintf(avcMsg, max_msg_char, "Not all EKF parameters could be read from the file. Terminating.");
        aviFail[0]= 1;
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free)(acados_ocp_capsule);
        EXPAND_AND_CONCAT(OPC_PREFIX,_acados_acados_free_capsule)(acados_ocp_capsule);
        acados_ocp_capsule= nullptr;
...
void step_MPC(avrSwap_t *avrSwap, int32_t *aviFail, char *accInfile, char *avcOutname, char *avcMsg, bool first_run) {
    if(first_run) {
        ...
        double fixedQxx[]= {0.0, avrSwap->wind_speed_hub*4.2642e-06, 0.0, 0.0}; // TODO: implement moving turbulence estimator
        for(int i=0; i<ekf.nbrstates; ++i)
            ekf.fixedQxx(i)= fixedQxx[i];

        snprintf(avcMsg, avrSwap->max_msg_char,
            "Initalizing DISCON with Tgen=%f, theta=%f, phi_rot_d=%f, vwind=%f, fixedQxx(%d)=%f",
            Tgen, theta, ekf.system.states.phi_rot_d, ekf.system.states.vwind, 2, fixedQxx[1]);
        aviFail[0]= 1;       
    }
    ...
    if((k_step%mpc_rate_f)==0) {
        ...
        const TrackingReferences tracking_ref = calc_tracking_references(
            ekf.system.states.vwind,
            ocp_params);
        ...
        yref[ocp_state_Tgen_idx] = tracking_ref.Tgen_ref;
        yref[ocp_state_theta_idx] = tracking_ref.theta_ref;
        yref[ocp_state_phi_rot_d_idx] = tracking_ref.om_rot_ref;
        ...
        if(k_step==0) {
            ...
            ocp_nlp_out_get(nlp_config, nlp_dims, nlp_out, nlp_in, N, "x", x0);
            ocp_nlp_out_set(nlp_config, nlp_dims, nlp_out, nlp_in, N-1, "x", x0);
            ocp_nlp_out_set(nlp_config, nlp_dims, nlp_out, nlp_in, 0, "x", x0);
        }*/
        
        int status;
        int wind_idx= ocp_params.info_map["vwind"].offset;
        if (wind_idx < 0 || wind_idx >= NP) {
            snprintf(avcMsg, avrSwap->max_msg_char,
                "Invalid sparse parameter index for vwind: %d (NP=%d). Parameter header and acados solver are inconsistent.",
                wind_idx, NP);
            aviFail[0]= 1;
            return;
        }
        for (int i = 0; i <= N; i++) {
            ...
            if (status) {
                snprintf(avcMsg, avrSwap->max_msg_char, "Setting wind parameter returned status %d in step %d.", status, k_step);
                aviFail[0]= 1;
            }
        }
        ...
        if(status!=0 && status!=2) {
            snprintf(avcMsg, avrSwap->max_msg_char, EXPAND_AND_STRINGIFY(OPC_PREFIX)"_acados_acados_solve() returned status %d in step %d.", status, k_step);
            aviFail[0]= 1;
        }
