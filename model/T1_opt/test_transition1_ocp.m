...
x0= [rated_torque 3 om_gen om_rot rated_torque/5.68E8 0.3 0 rated_torque*0.95];
...
ocp.constraints.lbx = [-acados_inf, beta_min, M_gen_min];
ocp.constraints.ubx = [M_gen_avg_max, beta_max, M_gen_max];
ocp.constraints.idxbx = [7 1 0];
...
subplot(6, 1, 2)
plot(t, simX(:, 3:4)/pi*30, t, t*0+OmMax/pi*30)
title('speed')
legend('generator speed', 'rotor speed', 'set point')
grid on

subplot(6, 1, 3)
plot(t(1:end), simX(:, 1), t(1:end), simX(:, 8), t(1:end), simX(:, 1)*0+TorqueMax)
