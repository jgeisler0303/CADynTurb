...
if 0
    ...
    W_x = diag([0, 0, 10, 10]); % setting to [0, 0, 10, 10] yields a seg fault in ocp_nlp_approximate_qp_matrices
    W_u = diag([0 0.001]);
...
% bound on u
ocp.constraints.lbu = [0 -pi/2];
ocp.constraints.ubu = [45e3 0];
...
ocp_solver.set('nlp_solver_max_iter', 200)
...
for i= 1:N-1
    ocp_solver.set('cost_W', blkdiag(W_x, W_u), i);
    ocp_solver.set('cost_y_ref', [x0; 40e3; 0], i);
end
