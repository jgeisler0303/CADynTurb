...
#include "T1_direct.hpp"

bool simulate(T1 &system, FAST_Wind* wind, double ts, double tfinal, const std::string &discon_path, const std::string &out_file_name, FAST_Parent_Parameters &p, double rpm0);
bool DISCON_Step(double t, DISCON_Interface &DISCON, T1 &system);
void initTorquePitch(double vwind, double om_rot, double &torque, double &pitch, T1Parameters &sys_param, FAST_Parent_Parameters &p);
...
void setupOutputs(FAST_Output &out, T1 &system) {
...
typedef decltype(std::declval<T1>().param.cm_lut) MatCx;
