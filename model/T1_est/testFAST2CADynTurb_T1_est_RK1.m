...
files_to_generate= {'_ode1.hpp', '_param.hpp', 'model_indices.m', 'model_indices_ode1.m', 'model_parameters.m'};
...
v= 12;
i= find(ref_sims.vv==v & ref_sims.yaw==0, 1);

d_in= loadData(ref_sims.files{i});

ss1= std(d_in.Wind1VelX.Data);
param.fixedQxx(vwind_idx)= (ss1/400)^2;

[d_est_1, ~, ~, ~, ~, ~, Q, R, x_end_est, P_end]= run_simulation([model_name '_RK1'], d_in, param, [], 0, 2, [], []);
d_est_2                                         = run_simulation([model_name '_RK1'], d_in, param, [], 0, 2, Q, R, [], x_end_est, P_end);

plot_timeseries_multi({d_in, d_est_1, d_est_2}, {'RAWS', 'LSSTipVxa', 'YawBrTDxp'}, {'FAST', 'est 1', 'est 2'})
