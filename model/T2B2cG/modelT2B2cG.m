...
for i = 1:3
    ...
    blade(i).rotateLocalAxis('x', (i-1)*T2B2cG.sym(2)/3*T2B2cG.sym(pi))
...
for i = 1:3
    ...
    thrustForceRadius= T2B2cG.sym(2)/3*T2B2cG.params.Rrot;
    % flapwise
    ...
    Rtheta= blade(i).rotationMatrix([0 0 1], -T2B2cG.inputs.theta);
...
Rcone= hub.rotationMatrix('y', T2B2cG.params.cone);
Rtheta= blade(1).rotationMatrix('z', T2B2cG.inputs.theta);
...
T2B2cG.addOutput('bld_flp_acc', T2B2cG.removeEps(bld_accel(1)));
T2B2cG.addOutput('bld_edg_acc', T2B2cG.removeEps(bld_accel(2)));
