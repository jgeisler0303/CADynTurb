...
/* File generated form template cadyn_direct_params.hpp.tem on 2023-08-04 09:37:42+02:00. Do not edit! */
...
T2B2cGParameters::T2B2cGParameters() :
    info_map(),
    unsetParams(106) {
    int offset= 0;
...
void T2B2cGParameters::setParam(const std::string &name, const real_type *value) {
...
real_type T2B2cGParameters::getParam(const std::string &name) {
    auto it = info_map.find(name);
    if(it==info_map.end())    
        throw std::runtime_error("Unknown parameter \"" + name + "\".");
...
void T2B2cGParameters::setFromFile(const std::string &fileName) {
...
bool T2B2cGParameters::unsetParamsWithMsg() {
    if(unsetParams) {
...
void T2B2cGParameters::getParamArray(real_type *p) {
    for(auto const &i : info_map) {
        if(!i.second.isSet)
