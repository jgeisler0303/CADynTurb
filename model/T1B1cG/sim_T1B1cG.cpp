...
#include "T1B1cG_direct.hpp"

bool simulate(T1B1cG &system, FAST_Wind* wind, double ts, double tfinal, const std::string &discon_path, const std::string &out_file_name, FAST_Parent_Parameters &p, double rpm0);
bool DISCON_Step(double t, DISCON_Interface &DISCON, T1B1cG &system);
void initTorquePitch(double vwind, double om_rot, double &torque, double &pitch, T1B1cGParameters &sys_param, FAST_Parent_Parameters &p);
...
void setupOutputs(FAST_Output &out, T1B1cG &system) {
...
typedef decltype(std::declval<T1B1cG>().param.cm_lut) MatCx;
...
double Trot(double vwind, double om_rot, double pitch, T1B1cGParameters *param) {
...
void initTorquePitch(double vwind, double om_rot, double &torque, double &theta_deg, T1B1cGParameters &sys_param, FAST_Parent_Parameters &p) {
    theta_deg= 0.0;
