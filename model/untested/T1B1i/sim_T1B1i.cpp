...
#include "T1B1i_direct.hpp"

bool simulate(T1B1i &system, FAST_Wind* wind, double ts, double tfinal, const std::string &discon_path, const std::string &out_file_name, FAST_Parent_Parameters &p, double rpm0);
bool DISCON_Step(double t, DISCON_Interface &DISCON, T1B1i &system);
void initTorquePitch(double vwind, double om_rot, double &torque, double &pitch, T1B1iParameters &sys_param, FAST_Parent_Parameters &p);
...
void setupOutputs(FAST_Output &out, T1B1i &system) {
    ...
    out.addChannel("RootMyb1", "kNm", &system.y[2], 1.0/1000.0);
    out.addChannel("RootMxb1", "kNm", &system.y[3], 1.0/1000.0);
    out.addChannel("RootMyb2", "kNm", &system.y[4], 1.0/1000.0);
    out.addChannel("RootMxb2", "kNm", &system.y[5], 1.0/1000.0);
    out.addChannel("RootMyb3", "kNm", &system.y[6], 1.0/1000.0);
    out.addChannel("RootMxb3", "kNm", &system.y[7], 1.0/1000.0);
}
...
double Trot(double vwind, double om_rot, double pitch, T1B1iParameters *param) {
...
void initTorquePitch(double vwind, double om_rot, double &torque, double &theta_deg, T1B1iParameters &sys_param, FAST_Parent_Parameters &p) {
    theta_deg= 0.0;
